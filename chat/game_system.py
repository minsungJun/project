...
class System:
    ...
    # 안넣은 기능 : 야추 중복 판별
    # 구현한 기능 : 리롤 횟수 제한, 턴 체크, 턴 넘기기, 점수 중복 입력 방지, 점수 미리 보기(미완)
    # 턴 넘기기 기능 구현할 때, 입력값 넣어서 점수 미리 보기 인지 아닌지 확인 하는거 해야됨

    def triple(self):
        if self.used_score[7] != True:
            total = 0
            temp = copy.deepcopy(self.dice)
            temp.sort()
